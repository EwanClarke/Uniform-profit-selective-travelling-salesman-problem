...
def construct_networkx_visualisation_positional(adjacency_matrix, no_of_nodes, path_indexes, pos):
    for i, row in enumerate(adjacency_matrix):
        for j in range(0, len(row)):
            adjacency_matrix[i][j] = round(adjacency_matrix[i][j], 2)
    ...
    for i in range(0, len(path_indexes) - 1):
        current_node = path_indexes[i]
        next_node = path_indexes[i + 1]
        solution_weights[(current_node, next_node)] = adjacency_matrix[current_node][next_node]
